...
export function ProjectStatsList({ stats, selectedProject, onSelectProject }: ProjectStatsListProps) {
  const allTotal = stats.reduce((s, st) => s + st.totalHours, 0);
  const allEst = stats.reduce((s, st) => s + st.estHours, 0);
  const allIssueCount = stats.reduce((s, st) => s + st.issueCount, 0);
